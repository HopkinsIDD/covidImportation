...
get_global_cum <- function(df = jhucsse, case_limit=100){
  ...
  conf_cases_global_cum <- conf_cases_global %>% group_by(Province_State) %>% 
    summarise(cum_cases = sum(Incidence)) %>% filter(cum_cases>=case_limit)
