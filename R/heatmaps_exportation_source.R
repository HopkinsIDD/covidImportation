...
coord_ratio <- length(unique(import_results_sourcetime$source)) / (length(unique(import_results_sourcetime$t))) / 5
#print(coord_ratio)
...
plot(ggheat_export(data=import_results_sourcetime %>% select(t, source, value=RR_mean) %>% 
                  rowwise %>% filter(as.Date(t) >= as.Date(t_limits[1]) & as.Date(t) <= as.Date(t_limits[2])),
                       zCuts=c(0,.01,.05,.1,.2,.5,.8,1.25,2,5,10,20,50,100,200,500),
                       x.size=8, y.size=8, title='Daily RR\nCase\nimported\nto Shenzhen', 
                       labCol='t', na.value="grey75",
                  aspect_ratio = coord_ratio))
ggsave(file.path("figures",project_name,sprintf("heatmap_RR_importation_v%s.png", version)), width = 6, height = 8, dpi=600)
...
plot(ggheat_export(data=import_results_sourcetime %>% select(t, source, value=prob_any_import) %>%
                           rowwise %>% filter(as.Date(t) >= as.Date(t_limits[1]) & as.Date(t) <= as.Date(t_limits[2])), 
              zCuts=seq(0,1,.1), x.size=8, y.size=8, 
              title='Daily\nProbability\nCase\nimported\nto Shenzhen', 
              na.value="grey85", pal_colors=c("lightgoldenrod1","darkorange1","darkred"),
              aspect_ratio = coord_ratio))

ggsave(file.path("figures",project_name,sprintf("heatmap_PROB_importation_v%s.png", version)), width = 6, height = 8, dpi=600)
...
plot(ggheat_export(data=import_results_sourcetime %>% select(t, source, value=import_mean) %>% 
                       rowwise %>% filter(as.Date(t) >= as.Date(t_limits[1]) & as.Date(t) <= as.Date(t_limits[2])),
                   zCuts=c(0,.01,.1,.5,1,2,4,6),
