...
travel_data_monthly <- read_csv(paste0("data/", destination, "-", dest_aggr_level, "_oag_20172019.csv"), na=c(""," ","NA"))
...
# # subset to 10 cities with most travel
# cities_sub <- ca_cities$arr_city[ca_cities$n_occur>=2000]
# #cities_sub <- c("San Francisco", "Oakland", "Sacramento", "San Diego", "San Jose", "Los Angeles")
# travel_data_monthly <- travel_data_monthly %>% filter(arr_city %in% cities_sub)
...
travel_data_monthly <- travel_data_monthly %>% 
  dplyr::select(source, destination=arr_metro, t_month, t_year, travelers)
...
# Limit to sources with cases --> speeds it up a lot
...
meanD_mat <- make_meanD(input_data_cases, n_sim, 
                        ...
                        inf_period_nohosp_mean, inf_period_nohosp_sd)
dim(meanD_mat)
dim(input_data_cases)
dim(travel_data_monthly)
dim(travel_data)
...
importation_sim <- run_daily_import_model_timeloc_withdailyest(
  ...
  meanD_mat=meanD_mat, t_red, u_origin, time_inftotravel, time_inftodetect,
...
coord_ratio <- length(unique(data$destination)) / (length(unique(data$t))) /.01
...
```{r plot_importsB, echo=FALSE, message=FALSE, warning=FALSE, fig.height=4, fig.width=8, eval=FALSE}
...
p_stackedsource_barchart <- plot_imports_facetsource(
  ...
  y.size = 8, x.size = 8, 
  leg.text.size = 8, leg.title.size = 8,
  rep_col = "indianred2", est_col = "blue3", ncol_legend=1, ncol_facet=2, scales="fixed") +
  theme(legend.position = "none")
