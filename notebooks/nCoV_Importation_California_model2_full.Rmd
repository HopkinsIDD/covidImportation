---
title: "COVID-19 Importation - California, All Sources"
...
# Analysis Options
project_name <- "california_import_v2"
version <- "wuhan_only_model2"
batch <- "1st"
n_sim <- 100
...
source("R/import_model_source_v2.R")
...
incid_data_orig <- incid_data

# ~ build the infecteds data using the fitted incidence data
incid_data <- build_infecteds(incid_data, 
                              incub_mean_log, incub_sd_log, 
                              inf_period_hosp_mean_log, inf_period_hosp_sd_log)

# ## Rename variables to work with model
# incid_data <- incid_data %>% rename(case)
...
plot_incidence_ests_report(conf_cases=jhucsse, 
                           incid_ests=incid_data_orig %>% rename(Incidence = cases_incid, Province_State=source),
                           locations="Hubei") +
...
travel_data_monthly <- travel_data_monthly %>% rowwise() %>% 
  mutate(travelers=ifelse(t_month == "01", travelers*1.6, travelers))

travel_data_daily <- make_daily_travel(travel_data=travel_data_monthly, travel_dispersion = 3)

# Hubei Travel shutdown
hubei_shutdown <- "2020-01-25" # it might have been Jan 23, but we will give it 1 day for people already departed.
...
#  - Stop travel after Jan 25 until May (crude)
...
start_date <- min((incid_data %>% filter(cases_curr_total>0))$t)
...
summ_dat <- input_data %>% group_by(destination, t) %>% summarise(travel_sum=sum(travelers, na.rm = TRUE),
                                                                  cases_incid_sum=sum(cases_inf_rep, na.rm = TRUE))
...
input_data <- readr::read_csv(file.path("data", project_name, 
                                        sprintf("input_data_%s_batch_v%s.RData", batch, version))) %>% as.data.frame()
# make sure no weird rows
input_data <- input_data %>% filter(!is.na(destination))

# importation_sim <- run_daily_import_model_timeloc_withdailyest(input_data, 
...
importation_sim <- run_daily_import_timeloc_model2(input_data, 
                                                   travel_data_monthly, travel_dispersion=3,
...
t_limits <- as.Date(c("2020-01-01","2020-03-15"))
