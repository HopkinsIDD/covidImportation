...
# Analysis Options
project_name <- "california_global_import"
version <- "global"
batch <- "1st"
n_sim <- 100
...
inf_period_hosp_mean_log <- 1.23 # increased based on Shenzhen (this is going to be a bit fluid)
...
incid_data <- est_daily_incidence_corrected(jhucsse, 
                                  ...
                                  last_date = ISOdate(2020,05,01),
                                  tol=50, na_to_zeros=FALSE)
...
incid_data <- incid_data %>% mutate(country_names=Country_Region) %>% 
  mutate(country_names = ifelse(country_names %in% c("Mainland China", "Hong Kong", "Macau", "Taiwan", "Nei Mongol"), "China", country_names))
incid_data <- incid_data %>% mutate(country_names = ifelse(source %in% c("Nei Mongol"), "China", country_names))

incid_data$country <- get_iso(incid_data$country_names)
...
These are the reported case counts by date of confirmation. These remain dominated by cases in Hubei.    
...
p_big <- ggplot(data=conf_cases_global %>% filter(Province_State %in% conf_cases_global_cum$Province_State), 
        ...
        facet_wrap(vars(Province_State), ncol=2)
plot(p_big)
...
plot_incidence_ests_report(conf_cases=jhucsse, 
                           incid_ests=incid_data %>% rename(Incidence = cases_incid, Province_State=source),
                           locations=unique(conf_cases_global_cum$Province_State)) +
  ggtitle("Global Corrected Incidence Estimates")
```
...
travel_data_monthly <- read_csv("data/cali_oag_20172019_aggr.csv", na=c(""," ","NA"))
...
sum(is.na(travel_data_monthly$dep_loc_aggr))
#View(travel_data_monthly[is.na(travel_data_monthly$dep_loc_aggr),])


# # Get Daily travel from the monthly
...
# Get rid for full country populations for those with subnational
pop_data <- pop_data %>% 
  filter(!(source %in% c("CHN", "USA")))
...
travel_data <- travel_data %>% mutate(source = dep_loc_aggr)
sum(is.na(travel_data$source))
sum(is.na(travel_data_monthly$dep_loc_aggr))
...
# SUMMARIZE DATA ----------------------------------------------------------
summ_dat <- input_data %>% group_by(destination, source, t) %>% 
  summarise(travel_sum=sum(travelers, na.rm = TRUE),
            cases_incid_sum=sum(cases_incid, na.rm = TRUE))

max((input_data %>% filter(dep_country=="USA"))$cases_incid, na.rm = TRUE)
...
meanD <- round(
  exp(rnorm(n_sim, mean = incub_mean_log, sd = incub_sd_log)) +
    p_report_source[1]*rlnorm(n_sim, meanlog=inf_period_hosp_mean_log, sdlog=inf_period_hosp_sd_log) +
...
time_inftodetect <- exp(rnorm(10000, mean = incub_mean_log, sd = incub_sd_log)) + 
  rlnorm(10000, meanlog=inf_period_hosp_mean_log, sdlog=inf_period_hosp_sd_log)
...
input_data <- readr::read_csv(file.path("data", project_name, 
                                        sprintf("input_data_%s_batch_v%s.RData", batch, version)))
#input_data <- input_data %>% rename(destination = )

# Travel restrictions
trav_ <- data.frame(loc=unique((input_data %>% filter(dep_country=="CHN"))$source), 
                    min=hubei_shutdown, max="2020-04-01", 
                    p_travel=.2) # Reduce travel from all chinese sources to 20%
travel_restrictions <- bind_rows(trav_ %>% filter(loc!="Hubei"),
                                 data.frame(loc="Hubei", min=hubei_shutdown, max="2020-04-01", p_travel=0))

travel_data_monthly <- travel_data_monthly %>% 
  dplyr::select(source=dep_loc_aggr, destination=arr_city, t_month, t_year, travelers)
